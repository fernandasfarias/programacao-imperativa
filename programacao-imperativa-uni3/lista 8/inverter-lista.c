...
void inverter(LISTA *l){
    ...
    for (int i = 0; i < j; i++){
        ...
        char valorAux[100];
        strcpy(valorAux, l->itens[i].valor);
        strcpy(l->itens[i].valor, l->itens[j].valor);
        strcpy(l->itens[j].valor, valorAux);

        int chaveAux = l->itens[i].chave;
        l->itens[i].chave = l->itens[j].chave;
        l->itens[j].chave = chaveAux;
        j--; 
    }
